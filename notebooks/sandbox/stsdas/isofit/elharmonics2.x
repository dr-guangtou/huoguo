...
# EL_HARMONICS2 -- Decompose data vectors in harmonic components.
...
procedure el_harmonics2 (bufx, bufy, ndata, coef, cerror, norder, rms, eps, err)
...
begin
	...
	do i = 1, ndata {
	    ...
	    if (!IS_INDEFR (bufy[i])) {
	        ...
	        do j = 0, n
		    #---------------------------------   LINE 74 ----------------------------------------------
	            # Default expression commented out below 
		    #FBF(j) = double (sin (FHN(j) * bufx[i] + PI2 * float(mod(j,2))))
	       	    # First-attempt expression commented out below
		    #FBF(j) = double (sin (-1.0*FHN(j) * atan(tan(bufx[i] )/(1.0 - eps)) + PI2 * float(mod(j,2))))
 
		    # Define the new angle psi, in the correct quadrant
		    if ((bufx[i] > PI2) && (bufx[i] < 3.0 * PI2)) 
			#psi=-1.0 * atan(tan(bufx[i] )/(1.0 - eps)) + float(2.0*PI2)
			FBF(j) = double (sin (FHN(j) * (-1.0 * atan(tan(bufx[i] )/(1.0 - eps)) + float(2.0*PI2))  + PI2 * float(mod(j,2))))
			#FBF(j) = double (cos (FHN(j) * (-1.0 * atan(tan(bufx[i] )/(1.0 - eps)) + float(2.0*PI2))))
		    else 
			#psi=-1.0 * atan(tan(bufx[i] )/(1.0 - eps))
			FBF(j) = double (sin (FHN(j) * (-1.0 * atan(tan(bufx[i] )/(1.0 - eps)))  + PI2 * float(mod(j,2))))
			#FBF(j) = double (cos (FHN(j) * (-1.0 * atan(tan(bufx[i] )/(1.0 - eps))) ))
		    #FBF(j) = double (sin (FHN(j) * psi  + PI2 * float(mod(j,2))))
	...
	do i = 1, ndata {
		...
		if ((bufx[i] > PI2) && (bufx[i] < 3.0 * PI2)) 
                    #psi=-1.0 * atan(tan(bufx[i] )/(1.0 - eps)) + float(2.0*PI2)
		    yc = yc + real (FRSIDE(j)) * sin (FHN(j) * (-1.0 * atan(tan(bufx[i] )/(1.0 - eps)) + float(2.0*PI2)) + PI2 * float(mod(j,2)))
                else 
                    #psi=-1.0 * atan(tan(bufx[i] )/(1.0 - eps))
		    yc = yc + real (FRSIDE(j)) * sin (FHN(j) * (-1.0 * atan(tan(bufx[i] )/(1.0 - eps))) + PI2 * float(mod(j,2)))
		
	        #yc = yc + real (FRSIDE(j)) * sin (FHN(j) * psi + PI2 * float(mod(j,2)))
#--------------------------------   LINE 121 -------------------------------------------------------
#	        yc = yc + real (FRSIDE(j)) *
